...
# GÖREV 1 ÇÖZÜMÜ:
new_cols1 = ["NUM_" + col.upper() if df[col].dtype != "O" else col.upper() for col in df.columns]
print(new_cols1)
...
# GÖREV 2 ÇÖZÜMÜ:
new_cols2 = [col.upper() + "_FLAG" if "no" not in col else col.upper() for col in df.columns]
print(new_cols2)
...
# GÖREV 3 ÇÖZÜMÜ:
new_cols3 = [col for col in df.columns if col not in og_list]
new_df = df[new_cols3]
print(new_df.head())
